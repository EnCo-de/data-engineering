...
class Middleware:
    ...
    def write(self, s):
        self.checksum.append(sum(s))
        self.sum += self.checksum[-1]
...
def process_payments():
    ...
    mock_storage = Middleware(storage)
    stream_payments_to_storage(mock_storage)
    # Here print the check sum of all of the bytes written by
    # `stream_payments_to_storage()`
    print('the check sum =', mock_storage.sum)
    print('optional, each separate one ', mock_storage.checksum)
...
def process_payments():
    ...
    generated by the `stream_payments_to_storage()` call.
    """
    mock_storage = io.BytesIO()
    storage = get_payments_storage()
    stream_payments_to_storage(mock_storage)
    v = mock_storage.getvalue()
    print(sum(v))
    storage.write(v)
...
def process_payments():
    ...
    # Here print the check sum of all of the bytes written by
    # `stream_payments_to_storage()`
    print(sum(mock_storage.read()))
    mock_storage.seek(0)
    storage.write(mock_storage.read())
